...
import ir.stackcode.iprotect.activities.MainActivity;
import ir.stackcode.iprotect.helpers.HelperSimCard;
...
public class BlackHoleService extends VpnService {
    ...
    private ParcelFileDescriptor vpnStart() {
        ...
        for (Rule rule : Rule.getRules(this))
            if (!(wifi ? rule.wifi_blocked : rule.other_blocked) && HelperSimCard.INSTANCE.isAllowed(this)) {
                Log.i(TAG, "Allowing " + rule.info.activityInfo.packageName);
                try {
                    builder.addDisallowedApplication(rule.info.activityInfo.packageName);
